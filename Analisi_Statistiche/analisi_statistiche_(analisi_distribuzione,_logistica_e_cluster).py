# -*- coding: utf-8 -*-
"""Tesi_Gamberini: Analisi Statistiche (Analisi distribuzione, logistica e cluster)
...
url_data_dummy = 'https://raw.githubusercontent.com/FedeGambe/Master_s_thesis_Data_science/750651749f45a7eb42c620ca985d178202b0c389/Dataset/dataset_dummy.csv'
data= pd.read_csv(url_data_dummy)

url_data_originale= 'https://raw.githubusercontent.com/FedeGambe/Master_s_thesis_Data_science/750651749f45a7eb42c620ca985d178202b0c389/Dataset/dataset_originale.csv'
data_originale= pd.read_csv(url_data_originale)

url_data_ricercatore = 'https://raw.githubusercontent.com/FedeGambe/Master_s_thesis_Data_science/750651749f45a7eb42c620ca985d178202b0c389/Dataset/Dataset_partenza%20-%20Dataset_partenza.csv'
data_ricercatore = pd.read_csv(url_data_ricercatore)

#Caricamenti da G Drive
"""
...
"""# **2. Analisi della distribuzione delle variabili**
...
fig_var_gen.update_traces(textinfo='percent+label', textfont=dict(size=18))
fig_var_gen.update_layout(title =
                 {'text': "Distribuzione della variabile Genere",'x': 0.5, 'xanchor': 'center','font': {'size': 20, 'family': 'Arial', 'weight': 'bold'}},
                  height=600,width=600, font=dict(size=16))
fig_var_gen.show()
...
fig_var_eta.update_layout(
    ...
    title={'text': "Distribuzione della variabile Classe d'età",'x': 0.5, 'xanchor': 'center','font': {'size': 20, 'family': 'Arial', 'weight': 'bold'}},
    height=600,width=1200,font=dict(size=16))
...
for i, p in enumerate(livello_istruzione_counts.values):
    percentage = f'{100 * p / total:.1f}%'
    fig_var_ist.add_annotation(x=livello_istruzione_counts.index[i],y=p + 200, text=percentage,showarrow=False,font=dict(size=14))
...
for i, p in enumerate(numero_persone_counts.values):
    percentage = f'{100 * p / total:.1f}%'
    fig_var_nfam.add_annotation(x=numero_persone_counts.index[i], y=p + 100, text=percentage, showarrow=False, font=dict(size=14))
...
fig_var_nfam.update_layout(
    ...
    title =
     {'text': "Distribuzione della variabile Numero persone in famiglia",
      'x': 0.5, 'xanchor': 'center','font': {'size': 20, 'family': 'Arial', 'weight': 'bold'}},
    height=600, width=1200, xaxis=dict(tickmode='array',tickvals=[str(i) for i in range(1, 14)]),font=dict(size=16))
fig_var_nfam.show()
...
for i, p in enumerate(numero_auto_counts.values):
    percentage = f'{100 * p / total:.1f}%'
    fig_var_nauto.add_annotation(x=numero_auto_counts.index[i], y=p + 200, text=percentage, showarrow=False, font=dict(size=14))
...
fig_var_nauto.update_layout(
    ...
    title =
     {'text': "Distribuzione della variabile Numero di auto in famiglia",
      'x': 0.5, 'xanchor': 'center','font': {'size': 20, 'family': 'Arial', 'weight': 'bold'}},
    height=700, width=1200, xaxis=dict(tickmode='array', tickvals=[str(i) for i in range(1, 6)]),font=dict(size=16))
fig_var_nauto.show()
...
for i in range(len(cross_tab.index)):
    for j in range(len(cross_tab.columns)):
        ...
        fig_ctab.add_annotation(x=j, y=i, text=f'{cross_tab.iloc[i, j]:.2%}', showarrow=False, font=dict(size=12, color=text_color))
...
fig_var_sens_2.update_traces(marker=dict(line=dict(color="black", width=0.5)))

fig_var_sens_2.update_layout(
    title={
        'text': "Distribuzione della variabile Importanza di ridurre le emissioni di gas serra",
        'x': 0.5,
        'xanchor': 'center',
...
"""# **3. Analisi della distribuzione delle variabili indipendenti in relazione con la dipendente**
...
### 3.2.1. Stacked plot qualitativi `(y= BEV dummy percent)`

```
# Calcolare le percentuali direttamente senza passaggi intermedi
percentuali_gen = data_originale.groupby(['BEV dummy', 'Genere']).size() \
    .unstack(fill_value=0) \
    .div(data_originale.groupby('Genere').size(), axis=1) \
    .mul(100) \
    .stack() \
    .reset_index(name='Percentuale')

#Secondo modo:
genere_bev_counts = data_originale.groupby(['BEV dummy', 'Genere']).size().unstack(fill_value=0)
totali_genere = genere_bev_counts.sum(axis=0) #fa ai totali del genere, mascho e femmina: 8019 e 2669
percentuali_gen = genere_bev_counts.div(totali_genere, axis=1) * 100
percentuali_gen = percentuali_gen.stack().reset_index(name='Percentuale')
percentuali_gen
```
"""

def calcola_percentuali(data, variabile):
    """
    Calcola le percentuali per una variabile rispetto a 'BEV dummy' in un DataFrame.

    :param data: DataFrame contenente i dati.
    :param variabile: Il nome della variabile categoriale per cui calcolare le percentuali (string).
    :return: DataFrame con le percentuali calcolate.
    """
    counts = data.groupby(['BEV dummy', variabile]).size().unstack(fill_value=0)     # Calcolare i conteggi per le combinazioni di 'BEV dummy' e la variabile
    totali = counts.sum(axis=0)     # Calcolare il totale per ciascun livello della variabile
    percentuali = counts.div(totali, axis=1) * 100    # Calcolare le percentuali
    percentuali = percentuali.stack().reset_index(name='Percentuale')     # Trasformare in formato "long" e aggiungere la colonna 'Percentuale'

    return percentuali

percentuali_gen = calcola_percentuali(data_originale, 'Genere')
percentuali_eta = calcola_percentuali(data_originale, 'Classe d\'età')
percentuali_reddito = calcola_percentuali(data_originale, 'Classe Reddito Familiare')
percentuali_istruzione = calcola_percentuali(data_originale, 'Livello di istruzione')
percentuali_casa_proprieta = calcola_percentuali(data_originale, 'Casa di proprietà')
percentuali_casa_indipendente = calcola_percentuali(data_originale, 'Casa Indipendente')
percentuali_auto_precedente = calcola_percentuali(data_originale, 'Tipologia di auto precedente')

#percentuali_eta["BEV dummy"] = percentuali_eta["BEV dummy"].astype(int)

color_map = {0: '#EF553B', 1: '#636EFA'}

fig_quali1_stacked = px.bar(data_frame=percentuali_gen,x="Genere",y="Percentuale",color="BEV dummy")
fig_quali1_stacked.update_traces(marker=dict(color=[color_map[val] for val in percentuali_gen["BEV dummy"]]))
fig_quali1_stacked.update_layout(barmode='group',title="Genere",height=400,width=550,xaxis=dict(tickfont=dict(size=16)),yaxis=dict(tickfont=dict(size=16)))

fig_quali2_stacked = px.bar(data_frame=percentuali_eta,x="Classe d'età",y="Percentuale",color="BEV dummy",category_orders={"Classe d'età": order_età})
fig_quali2_stacked.update_traces(marker=dict(color=[color_map[val] for val in percentuali_eta["BEV dummy"]]))
fig_quali2_stacked.update_layout(barmode='group',title="Classe d'età",height=400,width=550,xaxis=dict(tickfont=dict(size=16)),yaxis=dict(tickfont=dict(size=16)))

fig_quali3_stacked = px.bar(data_frame=percentuali_reddito,x="Classe Reddito Familiare",y="Percentuale",color="BEV dummy",category_orders={"Classe Reddito Familiare": order_red})
fig_quali3_stacked.update_traces(marker=dict(color=[color_map[val] for val in percentuali_reddito["BEV dummy"]]))
fig_quali3_stacked.update_layout(barmode='group',title="Classe Reddito Familiare",height=400,width=550,xaxis=dict(tickfont=dict(size=16)),yaxis=dict(tickfont=dict(size=16)))

fig_quali4_stacked = px.bar(data_frame=percentuali_istruzione,x="Livello di istruzione",y="Percentuale",color="BEV dummy",category_orders={"Livello di istruzione": order_ist})
fig_quali4_stacked.update_traces(marker=dict(color=[color_map[val] for val in percentuali_istruzione["BEV dummy"]]))
fig_quali4_stacked.update_layout(barmode='group',title="Livello di istruzione",height=400,width=550,xaxis=dict(tickfont=dict(size=16)),yaxis=dict(tickfont=dict(size=16)))

fig_quali5_stacked = px.bar(data_frame=percentuali_casa_proprieta,x="Casa di proprietà",y="Percentuale",color="BEV dummy")
fig_quali5_stacked.update_traces(marker=dict(color=[color_map[val] for val in percentuali_casa_proprieta["BEV dummy"]]))
fig_quali5_stacked.update_layout(barmode='group',title="Casa di proprietà",height=400,width=550,xaxis=dict(tickfont=dict(size=16)),yaxis=dict(tickfont=dict(size=16)))

fig_quali6_stacked = px.bar(data_frame=percentuali_casa_indipendente,x="Casa Indipendente",y="Percentuale",color="BEV dummy")
fig_quali6_stacked.update_traces(marker=dict(color=[color_map[val] for val in percentuali_casa_indipendente["BEV dummy"]]))
fig_quali6_stacked.update_layout(barmode='group',title="Casa Indipendente",height=400,width=550,xaxis=dict(tickfont=dict(size=16)),yaxis=dict(tickfont=dict(size=16)))

fig_quali7_stacked = px.bar(data_frame=percentuali_auto_precedente,x="Tipologia di auto precedente",y="Percentuale",color="BEV dummy")
fig_quali7_stacked.update_traces(marker=dict(color=[color_map[val] for val in percentuali_auto_precedente["BEV dummy"]]))
fig_quali7_stacked.update_layout(barmode='group',title="Tipologia di auto precedente",height=400,width=550,xaxis=dict(tickfont=dict(size=16)),yaxis=dict(tickfont=dict(size=16)))

"""### 3.2.2. Plot qualitativi `(y=count)`"""

#Plot qualitativi singoli

fig_quali1=  px.histogram(data_frame=data_biva, x="Genere", color="BEV dummy")
fig_quali1.update_layout(barmode='group', title = "Genere", height=400, width=550,xaxis=dict(tickfont=dict(size=12)),yaxis=dict(tickfont=dict(size=12)))

fig_quali2 = px.histogram(data_frame=data_biva, x="Classe d'età", color="BEV dummy",category_orders={"Classe d'età": order_età})
fig_quali2.update_layout(barmode='group', title = "Classe d'età", height=400, width=550,xaxis=dict(tickfont=dict(size=12)),yaxis=dict(tickfont=dict(size=12)))
...
fig_quali3.update_layout(barmode='group', title = "Classe Reddito Familiare", height=400, width=550,xaxis=dict(tickfont=dict(size=12)),yaxis=dict(tickfont=dict(size=12)))
...
fig_quali4.update_layout(barmode='group', title = "Livello di istruzione", height=400, width=550, xaxis=dict(tickfont=dict(size=12)),yaxis=dict(tickfont=dict(size=12)))

fig_quali5 = px.histogram(data_frame=data_biva, x="Casa di proprietà", color="BEV dummy")
fig_quali5.update_layout(barmode='group', title = "Casa di proprietà", height=400, width=550,xaxis=dict(tickfont=dict(size=12)),yaxis=dict(tickfont=dict(size=12)))

fig_quali6 = px.histogram(data_frame=data_biva, x="Casa Indipendente", color="BEV dummy")
fig_quali6.update_layout(barmode='group', title = "Casa Indipendente", height=400, width=550,xaxis=dict(tickfont=dict(size=12)),yaxis=dict(tickfont=dict(size=12)))

fig_quali7 = px.histogram(data_frame=data_biva, x="Tipologia di auto precedente", color="BEV dummy")
fig_quali7.update_layout(barmode='group', title = "Tipologia di auto precedente", height=400, width=550, xaxis=dict(tickfont=dict(size=16)),yaxis=dict(tickfont=dict(size=16)))
...
fig_quant_bp = make_subplots(rows=2, cols=4, shared_xaxes=True, vertical_spacing=0.07, horizontal_spacing=0.07, subplot_titles=X_quant_abbr)
...
fig_quant1 = make_subplots(rows=4, cols=2, subplot_titles=X_quant_abbr, shared_xaxes=False, shared_yaxes=False,vertical_spacing=0.1, horizontal_spacing=0.1)
...
for i, var in enumerate(X_quant):# una riga per ogni variabile in X_quant_abbr
    ...
    for bev_value in [1, 0]:     # linee separate per i valori di BEV dummy (0 e 1)
        x_filtered = counts.index  # I valori della variabile X_quant
        y_filtered = counts[bev_value]  # Conteggio per BEV dummy
        fig_quant1.add_trace(
            go.Scatter(
                x=x_filtered, y=y_filtered,
                mode='lines+markers',
                line=dict(shape='spline', smoothing=1.0, width=2, color=colors[bev_value]),
    ...
    fig_quant1.update_xaxes(
        range=x_limits[var],  # Applica il limite specificato
        row=row, col=col)

fig_quant1.update_layout(title =
                 {'text': "Conteggi tra variabili quantitative e BEV dummy",
                  'x': 0.5, 'xanchor': 'center','font': {'size': 20, 'family': 'Arial', 'weight': 'bold'}},
    height=1200, width=1400,showlegend=True,)

fig_quant1.update_xaxes(title_text="Valore variabile", row=4, col=1)
fig_quant1.update_yaxes(title_text="Conteggio", row=4, col=1)
fig_quant1.show()

def create_bins(data, col_name, bins, suffix):
    bin_col_name = f'{col_name}_bin'
    bin_num_col_name = f'{col_name}_bin_num'
    data[bin_col_name] = pd.cut(data[col_name], bins=bins, right=False)     # Aggiungi la colonna dei bin
    data[bin_num_col_name] = data[bin_col_name].apply(lambda x: x.left)     # Aggiungi la colonna con il numero del bin (valore inferiore di ciascun intervallo)
    return data

bins_viaggi = np.arange(0, 1050, 50)
bins_nviaggi = np.arange(0, 12, 1)
bins_importanza = np.arange(-3, 3.5, 0.5)
bins_dist = np.arange(0, 60, 5)
bins_vmt = np.arange(0, 51000, 1000)

data_biva = create_bins(data_biva, 'Importanza di ridurre le emissioni di gas serra', bins_importanza, 'Importanza')
data_biva = create_bins(data_biva, 'Viaggio più lungo negli ultimi 12 mesi', bins_viaggi, 'viaggi')
data_biva = create_bins(data_biva, 'Numero di viaggi superiori a 200 miglia negli ultimi 12 mesi', bins_nviaggi, 'nviaggi')
data_biva = create_bins(data_biva, 'Distanza casa-lavoro', bins_dist, 'dist')
data_biva = create_bins(data_biva, 'VMT annuo', bins_vmt, 'vmt')

data_biva.head()

var_abbr = {
    'Numero persone in famiglia': "Num. persone famiglia",
    'Numero di auto in famiglia': "Num. auto famiglia",
    'Importanza di ridurre le emissioni di gas serra_bin_num': "Imp. riduzione emissioni",
    'Viaggio più lungo negli ultimi 12 mesi_bin_num': "Viaggio più lungo",
    'Numero di viaggi superiori a 200 miglia negli ultimi 12 mesi_bin_num': "N. viaggi > 200 miglia",
    'Distanza casa-lavoro_bin_num': "Distanza casa-lavoro",
    'VMT annuo_bin_num': "VMT annuo"}

vars_to_plot = [
    'Numero persone in famiglia',
    'Numero di auto in famiglia',
    'Importanza di ridurre le emissioni di gas serra_bin_num',
    'Viaggio più lungo negli ultimi 12 mesi_bin_num',
    'Numero di viaggi superiori a 200 miglia negli ultimi 12 mesi_bin_num',
    'Distanza casa-lavoro_bin_num',
    'VMT annuo_bin_num']

fig_quant = make_subplots(rows=4, cols=2, shared_xaxes=False, shared_yaxes=False, subplot_titles=[var_abbr[var] for var in vars_to_plot], vertical_spacing=0.07)

for idx, var in enumerate(vars_to_plot): # Creiamo un grafico per ogni variabile
    if '_bin_num' in var:    # Convertiamo i '_bin_num' in float
        data_biva[var] = data_biva[var].astype(float)
    count_data = data_biva.groupby([var, 'BEV dummy']).size().reset_index(name='count')     # Raggruppa i dati per ciascuna variabile e per BEV dummy
    data_bev_0 = count_data[count_data['BEV dummy'] == 0]
    data_bev_1 = count_data[count_data['BEV dummy'] == 1]
    row = idx // 2 + 1
    col = idx % 2 + 1
    show_legend = (idx == 0)     # Determiniamo se siamo sul primo grafico (indice 0)


    # Grafico per BEV dummy == 0
    fig_quant.add_trace(go.Scatter(
        x=data_bev_0[var], y=data_bev_0['count'], mode='lines+markers',
        name='BEV=0', line=dict(shape='spline', smoothing=1.0, width=2, color='blue'),
        fill='tozeroy', fillcolor='rgba(0, 0, 255, 0.1)', showlegend=show_legend), row=row, col=col)

    # Grafico per BEV dummy == 1
    fig_quant.add_trace(go.Scatter(
        x=data_bev_1[var], y=data_bev_1['count'], mode='lines+markers',
        name='BEV=1', line=dict(shape='spline', smoothing=1.0, width=2, color='red'),
        fill='tozeroy', fillcolor='rgba(255, 0, 0, 0.1)', showlegend=show_legend), row=row, col=col)

fig_quant.update_layout(title ={'text': "Conteggi delle variabili dip. quantitative in relazione con la y (BEV dummy)",
                  'x': 0.5, 'xanchor': 'center','font': {'size': 18, 'family': 'Arial', 'weight': 'bold'}},
    height=1200, width = 800,
    showlegend=True,
    legend_title="BEV",
    xaxis=dict(tickfont=dict(size=12)),yaxis=dict(tickfont=dict(size=12)),
    xaxis1=dict(tickfont=dict(size=12)),yaxis1=dict(tickfont=dict(size=12)),)
fig_quant.show()
...
"""
fig_quali1_stacked.write_image('Distribuzione_Genere_BEV_stacked.pdf')
fig_quali2_stacked.write_image('Distribuzione_Classe_età_BEV_stacked.pdf')
fig_quali3_stacked.write_image('Distribuzione_Classe_reddito_familiare_BEV_stacked.pdf')
fig_quali4_stacked.write_image('Distribuzione_Livello_istruzione_BEV_stacked.pdf')
fig_quali5_stacked.write_image('Distribuzione_Casa_proprietaria_BEV_stacked.pdf')
fig_quali6_stacked.write_image('Distribuzione_Casa_indipendente_BEV_stacked.pdf')
fig_quali7_stacked.write_image('Distribuzione_Tipologia_auto_precedente_BEV_stacked.pdf')
...
print(f"\nR2 di McFadden per modello completo: {mcfadden_rsquared_vif:.6f}")
...
print(f"\nR2 di McFadden per modello ridotto: {mcfadden_rsquared_vif2:.6f}")
print("\n===============================================================")
# Confronto
...
data['Probabilità'] = result.predict(X)
data.head()
...
print("Per una persona di classe Reddito Familiare Alta la probabilità rispetto ad una di classe media di acquistare un veicolo BEV è:",
      round(odds_reddito_am-1,5), ",ovvero la probabilità aumenta del",round(odds_reddito_am-1,4)*100, "%")
print()
...
print("Per una persona di classe Reddito Familiare Molto Alta la probabilità rispetto ad una di classe Media di acquistare un veicolo BEV è:",
      round(odds_reddito_mam-1,5), ",ovvero la probabilità aumenta del",round(odds_reddito_mam-1,4)*100, "%")
print()
...
fig = px.scatter(data, x='Distanza casa-lavoro', y='Probabilità',
                 title='Grafico Logistico della Distanza Casa-Lavoro',
                 labels={'Distanza casa-lavoro':'Distanza casa-lavoro (km)', 'Probabilità':'Probabilità'})
fig.show()
...
fig = px.scatter(data, x='Numero di viaggi superiori a 200 miglia negli ultimi 12 mesi', y='Probabilità',
                 title='Grafico Logistico della N viaggi')
fig.show()

"""#### 5.3.4.3. VMT"""

#odds_VMT = round(tabella_sign.loc[tabella_sign.index[18], 'Odds Ratio'],3)
odds_VMT = np.exp(-1.504e-05)
print(odds_VMT)
print()
VMT_calcolato = (-(1 - odds_VMT)*100) * data['VMT annuo']

print("ogni miglio percorse in più annualmente la probabilità scende di:" ,round((1-odds_VMT)*1,10), ", ovvero scende del:", round((1-odds_VMT)*1*100,10), "%")
...
fig = px.scatter(data, x='VMT annuo', y='Probabilità',
                 title='Grafico Logistico del VMT annuo')
fig.show()
...
fig_cm_mob1.update_layout(
    ...
    height=700,
    width=1200,
    font=dict(size=16),  # Modifica la dimensione del testo
    xaxis=dict(tickfont=dict(size=18)),  # Dimensione dei tick sull'asse x
    yaxis=dict(tickfont=dict(size=18))   # Dimensione dei tick sull'asse y
)
...
fig = px.scatter(data, x='Importanza di ridurre le emissioni di gas serra', y='Probabilità',
                 title='Grafico Logistico della sens. ambientale')
fig.show()
...
url_data_kmode_r = 'https://raw.githubusercontent.com/FedeGambe/Master_s_thesis_Data_science/cda2797794e15e08316b40d9d793bfdb95a2cedc/Dataset/data_kmode_R.csv'
data_kmode_r= pd.read_csv(url_data_dummy)
...
url_data_kmeans = 'https://raw.githubusercontent.com/FedeGambe/Master_s_thesis_Data_science/8c067305da4c19ff539dc3740f579a4f328930d0/Dataset/data_con_cluster.csv'
url_data_kmeans_2 = 'https://raw.githubusercontent.com/FedeGambe/Master_s_thesis_Data_science/8c067305da4c19ff539dc3740f579a4f328930d0/Dataset/data_con_cluster2.csv'
url_data_kmeans_org = 'https://raw.githubusercontent.com/FedeGambe/Master_s_thesis_Data_science/8c067305da4c19ff539dc3740f579a4f328930d0/Dataset/data_con_clusterorg.csv'

data_kmeans = pd.read_csv(url_data_kmeans)
data_kmeans_2 = pd.read_csv(url_data_kmeans_2)
data_kmeans_org = pd.read_csv(url_data_kmeans_org)
